...
<h1>Термины</h1>
<p>
<strong class="highlighter" data-highlighter="[data-immerser]">Корневой элемент иммёрсера</strong>&nbsp;&mdash; это родительский контейнер для ваших фиксированных блоков.
...
`,
  ...
  'option-onInit': 'Колбек после инициализации. Принимает единственный параметр — экземпляр иммёрсера',
  'option-onBind': 'Колбек после привязки к документу. Принимает единственный параметр — экземпляр иммёрсера',
  'option-onUnbind': 'Колбек после отвязки от документа. Принимает единственный параметр — экземпляр иммёрсера',
  'option-onDestroy': 'Колбек после уничтожения. Принимает единственный параметр — экземпляр иммёрсера',
  'option-onActiveLayerChange':
    'Колбек после смены активного слоя. Принимает первым параметром индекс следующего слоя, экземпляр иммёрсера вторым',
...
<p>
  Как упомянуто ранее, иммёрсер клонирует узлы документа, чтобы добиться смены по&nbsp;скроллу.
