...
Также вы&nbsp;можете передать конфигурацию всех слоев объектом в&nbsp;параметре <code>solidClassnamesByLayerId</code> настроек.
...
`,
  ...
  'option-solidClassnamesByLayerId':
    'Объект настроек слоев по id. Конфигурация, переданная в data-immerser-layer-config перезапишет эту настройку для соответствующего слоя. Пример конфигурации <a href="#initialize-immerser">показан выше</a>',
  'option-autoMount':
    'Запускает DOM-инициализацию из конструктора. Передайте false, чтобы вызвать mount вручную после готовности DOM',
  'option-selectorRoot': 'Родительская нода, ограничивающая поиск по селекторам во время mount. По умолчанию document',
  ...
  'option-hasExternalScroll':
    'Не управляет скроллом, если включен. Задуман для работы в связке с внешним контроллером скролла',
  'option-hasExternalRenderer':
    'Не запускает большую часть рутины с модификацией DOM, если включен. Задуман для работы в связке с рендер фреймворками типа React, Vue.js и другими',
  ...
  'event-bind': 'Вызывается после привязки к <abbr title="Document Object Model">DOM</abbr>',
  'event-unbind': 'Вызывается после отвязки от <abbr title="Document Object Model">DOM</abbr>',
  ...
  'public-field-mount': 'Ищет DOM, проверяет разметку, рассчитывает размеры и навешивает слушатели уровня mount',
  'public-field-enable': 'Включает рантайм: готовит разметку, наведение, навигацию и первую отрисовку',
  'public-field-disable': 'Выключает рантайм, очищает сгенерированную разметку и оставляет экземпляр смонтированным',
  'public-field-updateOptions': 'Обновляет рантайм-настройки и применяет минимальные побочные эффекты без remount',
  ...
  'public-field-addLayer': 'Добавляет слой и подготавливает его рантайм-разметку, если иммёрсер привязан',
  'public-field-removeLayer': 'Удаляет слой и принадлежащую ему рантайм-разметку',
  ...
  'public-field-isEnabled': 'Показывает, активен ли сейчас иммёрсер (разметка склонирована, подписчики навешаны)',
  'public-field-isMounted': 'Показывает, выполнен ли поиск DOM и активны ли слушатели уровня mount',
